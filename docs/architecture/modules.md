...
ESP-IDF Runtime 当前已经承接产品入口和第一阶段双核模型：`bike_runtime` 固定 Core 0，`bike_ui` 固定 Core 1，AI Assistant、Cloud Worker、Wi-Fi Worker 固定 Core 0。AudioSession codec/I2S 初始化已在 ESP-IDF 上到达 `session ready`；Qwen ASR/Qwen Chat 已有 ESP-IDF `esp_http_client` transport 构建路径；新功能迁入前仍需继续补齐 ESP-IDF TTS、取消、板级回归和资源基线。
